defmodule AdventOfCode.Y23.Day17 do
  ...
  def part_one(grid) do
    ...
    find_target(start_poses, target_xy, %{}, grid)
  end

  defp find_target([{target_xy, _, cost} | open], target_xy, seen, grid) do
    cost
  end

  defp find_target([h | open], target_xy, seen, grid) do
    {next_poses, seen} =
      next_poses(h, grid)
    ...
    open = insert_all(next_poses, open) |> Enum.take(1000)
    find_target(open, target_xy, seen, grid)
  end
  ...
  defp next_poses({xy, {dir, count}, cost}, grid) when count < 3 do
  ...
  defp next_poses({xy, {dir, 3}, cost}, grid) do
    poses = []
